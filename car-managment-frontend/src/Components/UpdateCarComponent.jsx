...
function UpdateCarComponent() {
    ...
    useEffect(() => {
        getCarById(id)
            .then(res => {
                ...
                setBrand(car.brand);
                setSAPNumber(car.sap);
                setInventoryNumber(car.inventoryNumber);
                setAmount(car.amount);
                setCarStatus(car.carStatus);
                setEnvironmentalCategory(car.environmentalCategory);
                setChassi(car.chassi);
                setMileage(car.mileage);
                setCity(car.city);
                setActivationDate(car.activationDate);
                setEndDate(car.endDate);
                setCarUsedFromDate(car.carUsedFromDate);
                setCarUsedToDate(car.carUsedToDate);
                setSalesValue(car.salesValue);
    ...
    const editCar = async (e) => {
        e.preventDefault();
        try {
            const car = { number, brand, sap, inventoryNumber, amount, carStatus, kW, environmentalCategory, chassi, mileage, city, activationDate, endDate, carUsedFromDate, carUsedToDate, salesValue, sellingPrice };
            await updateCar(car, id);
            toast.success('Car Insurance updated successfully!', successToastOptions);
            setTimeout(() => {
                navigate('/cars');
            }, 500); // Время в миллисекундах
        } catch (error) {
            console.error('Error updating Car Insurance: ', error);
            toast.error('Failed to update Car Insurance', errorToastOptions);
        }
    };
    ...
    return (
        <div className='d-flex align-items-center justify-content-center' style={{ minHeight: '100vh', backgroundColor: '#161718' }}>
            <div className='col-md-6' style={{ maxWidth: '600px', width: '100%', border: '2px solid #CCC', borderRadius: '10px', padding: '20px' }}>
                <h3 className='text-center' style={{ fontSize: '32px', marginBottom: '20px', color: '#00D8FF' }}>Update Car</h3>
                <div className='card p-4 border rounded'>
                    <form>
                        ...
                        <button className='btn btn-success' onClick={editCar} style={{ fontSize: '20px', marginTop: '10px' }}>Update</button>
