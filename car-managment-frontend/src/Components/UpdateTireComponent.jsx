...
import { getTireById, updateTire } from '../services/TireService';
...
function UpdateTireComponent() {
  ...
  useEffect(() => {
    getTireById(id)
        .then(res => {
            const tire = res.data;
            setPurchaseDate(tire.purchaseDate);
            setPrice(tire.price);
            setSupplier(tire.supplier);
            setSeason(tire.season);
            setSize(tire.size);
            setStorageLocation(tire.storageLocation);
            setChangeDate(tire.changeDate);
            setCarNumber(tire.carNumber);
        })
        .catch(error => console.error(error));
}, [id]);

const editTire = async (e) => {
    e.preventDefault();
    try {
        const tire = { purchaseDate, price, supplier, season, size, storageLocation, changeDate, carNumber };
        await updateTire(tire, id);
        toast.success('Tire updated successfully!', successToastOptions);
        setTimeout(() => {
            navigate('/tires');
        }, 500); // Время в миллисекундах
    } catch (error) {
        console.error('Error updating Tire: ', error);
        toast.error('Failed to update Tire', errorToastOptions);
    }
};
  ...
  return (
    <div className='d-flex align-items-center justify-content-center' style={{ minHeight: '100vh', backgroundColor: '#161718' }}>
      <div className='col-md-6' style={{ maxWidth: '600px', width: '100%', border: '2px solid #CCC', borderRadius: '10px', padding: '20px' }}>
        <h3 className='text-center' style={{ fontSize: '32px', marginBottom: '20px', color: '#00D8FF' }}>Update Tire</h3>
          ...
          <form>
            ...
            <div className='form-group'>
              ...
              />
                        ...
                        <button className='btn btn-success' onClick={editTire} style={{ fontSize: '20px', marginTop: '10px' }}>Update</button>
...
export default UpdateTireComponent